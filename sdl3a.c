...
typedef struct _player_ {
  ...
  // Make A Few Counters For Textures //
  // IDLE just needs 1 state //
  int counterIdle; 
  // MOVING needs 2 (body and feet) //
  int counterMoving;
...
void makeBullet(player *tha_playa, mouse *tha_mouse);
void travelBullet();
...
void showPlayerState(player *tha_playa) {
  ...
  switch(state) {
	...
	"gfx/idlebody/survivor-idle_handgun_18.png", "gfx/idlebody/survivor-idle_handgun_19.png"
      ...
      //tha_playa->txtr_feet = load_texture("gfx/idlefeet/survivor-idle_0.png");
...
void makeBullet(player *tha_playa, mouse *tha_mouse) {
  ...
  bulletPointer.x = tha_playa->x;
  bulletPointer.y = tha_playa->y;
  
  bulletPointer.factorX = (bulletPointer.x - tha_mouse->x) / 25;
  bulletPointer.factorY = (bulletPointer.y - tha_mouse->y) / 25; 
...
void travelBullet() {
  for (int a = 0; a < BULLET_COUNT; a++) {
    ...
    SDL_Rect dest;
    dest.x = shotBullet.x - shotBullet.factorX;
    dest.y = shotBullet.y - shotBullet.factorY;

    shotBullet.x = dest.x;
    shotBullet.y = dest.y;
    bullets[shotBullet.bulletCount] = shotBullet;

    blit_angled(shotBullet.txtr_bullet, shotBullet.x, shotBullet.y, shotBullet.angle);
  }
}

void handle_key(SDL_KeyboardEvent *keyevent, keystate updown, player *tha_playa) {
  if (keyevent->repeat == 0) {
    int pressedW = keyevent->keysym.scancode == SDL_SCANCODE_W; 
    int pressedS = keyevent->keysym.scancode == SDL_SCANCODE_S;
    int pressedA = keyevent->keysym.scancode == SDL_SCANCODE_A;
    int pressedD = keyevent->keysym.scancode == SDL_SCANCODE_D;
...
void process_input(player *tha_playa, mouse *tha_mouse) {	
  ...
  while (SDL_PollEvent(&event))	{		
	...
	makeBullet(tha_playa, tha_mouse);
	break;
